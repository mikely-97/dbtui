# Run Panel History Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Let users scroll back through previous dbt run outputs in the Run panel without losing the current output.

**Architecture:** `RunPanel` gains a `_history: list[dict]` attribute. After each run completes (success or failure), the current RichLog lines are snapshotted into `_history`. Two new navigation buttons (`← Prev` / `Next →`) restore a past snapshot into the RichLog. A counter label shows "run 2/3" or "live".

**Tech Stack:** Textual (`RichLog`, `Button`, `Static`, `Horizontal`), no new dependencies.
...
| File | Change |
|------|--------|
| `src/dbt_tui/frontend/model_view/run_panel.py` | Add history store + nav controls |
| `tests/test_runner.py` | No change needed — this is pure UI state |

---

### Task 1: Extend RunPanel with history

**Files:**
- Modify: `src/dbt_tui/frontend/model_view/run_panel.py`

- [ ] **Step 1: Read the current file**

File: `src/dbt_tui/frontend/model_view/run_panel.py`

Current layout:
```
Horizontal(btn-run, btn-test, btn-build, btn-cancel, run-status)  #run-controls
RichLog  #run-log
```

- [ ] **Step 2: Replace with history-aware version**
...
```bash
git add src/dbt_tui/frontend/model_view/run_panel.py
git commit -m "feat(run-panel): add run history with ← Prev / Next → navigation"
```
